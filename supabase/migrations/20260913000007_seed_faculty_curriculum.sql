-- Seeds four real-world courses with genuine, textbook-quality content (not lorem ipsum),
-- each published immediately so they appear in the instructor adoption catalog right away.
-- CIS 202W is READING-only; CIS 350, 631, and 483 mix READING (concept) and LAB (hands-on)
-- sections per chapter, matching the pattern established by the existing seeded courses.
--
-- Each course is one WITH-chain: INSERT ... RETURNING id feeds the next INSERT via a plain
-- SELECT, so chapter/section rows are correctly scoped to their parent without any procedural
-- code or manually-tracked variables.

-- =====================================================================================
-- CIS 202W - Computer In Society
-- =====================================================================================
WITH course_202w AS (
  INSERT INTO public.courses (title, description, is_published)
  VALUES (
    'CIS 202W - Computer In Society',
    'A survey of the ethical, legal, and social dimensions of computing: privacy, algorithmic fairness, and the digital divide.',
    true
  )
  RETURNING id
),
ch1_202w AS (
  ...
  SELECT id, 'Chapter 1: Foundations of Digital Ethics', 1 FROM course_202w
  RETURNING id
),
ch2_202w AS (
  INSERT INTO public.chapters (course_id, title, display_order)
  SELECT id, 'Chapter 2: Technology''s Impact on Society', 2 FROM course_202w
  RETURNING id
)
...
WITH course_350 AS (
  ...
  VALUES (
    'CIS 350 - Information Security',
    'Core principles of information security, from threat modeling and cryptography to common web application vulnerabilities.',
    true
  )
  RETURNING id
),
ch1_350 AS (
  INSERT INTO public.chapters (course_id, title, display_order)
  SELECT id, 'Chapter 1: Security Fundamentals', 1 FROM course_350
  RETURNING id
),
ch2_350 AS (
  INSERT INTO public.chapters (course_id, title, display_order)
  SELECT id, 'Chapter 2: Network and Application Security', 2 FROM course_350
  RETURNING id
)
INSERT INTO public.sections (chapter_id, title, content_type, markdown_content, display_order)
...
-- =====================================================================================
-- CIS 631 - Machine Learning
-- =====================================================================================
WITH course_631 AS (
  INSERT INTO public.courses (title, description, is_published)
  VALUES (
    'CIS 631 - Machine Learning',
    'Graduate introduction to supervised learning: regression, classification, and model evaluation, with hands-on implementation.',
    true
  )
  RETURNING id
),
ch1_631 AS (
  INSERT INTO public.chapters (course_id, title, display_order)
  SELECT id, 'Chapter 1: Foundations of Supervised Learning', 1 FROM course_631
  RETURNING id
),
ch2_631 AS (
  INSERT INTO public.chapters (course_id, title, display_order)
  SELECT id, 'Chapter 2: Classification and Model Evaluation', 2 FROM course_631
  RETURNING id
)
INSERT INTO public.sections (chapter_id, title, content_type, markdown_content, display_order)
...
-- =====================================================================================
-- CIS 483 - Web application design and development
-- =====================================================================================
WITH course_483 AS (
  INSERT INTO public.courses (title, description, is_published)
  VALUES (
    'CIS 483 - Web application design and development',
    'Full-stack web development covering component-based front-end architecture and RESTful back-end API design.',
    true
  )
  RETURNING id
),
ch1_483 AS (
  INSERT INTO public.chapters (course_id, title, display_order)
  SELECT id, 'Chapter 1: Front-End Fundamentals', 1 FROM course_483
  RETURNING id
),
ch2_483 AS (
  INSERT INTO public.chapters (course_id, title, display_order)
  SELECT id, 'Chapter 2: Back-End Fundamentals and APIs', 2 FROM course_483
  RETURNING id
)
INSERT INTO public.sections (chapter_id, title, content_type, markdown_content, display_order)
